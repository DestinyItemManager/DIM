...
function StatsCtrl(this: IController, $scope: IScope, $i18next) {
  ...
  $scope.$watch('vm.stats', () => {
    ...
    vm.statList =
      vm.destinyVersion === 1
        ? [vm.stats.STAT_INTELLECT, vm.stats.STAT_DISCIPLINE, vm.stats.STAT_STRENGTH]
        : [vm.stats.maxBasePower, vm.stats[2996146975], vm.stats[392767087], vm.stats[1943323491]];
    vm.statList.forEach((stat) => {
      if (stat) {
        // compute tooltip
        if (vm.destinyVersion === 1) {
          const tier = stat.tier || 0;
          const next = $i18next.t('Stats.TierProgress', {
            context: tier === 5 ? 'Max' : '',
