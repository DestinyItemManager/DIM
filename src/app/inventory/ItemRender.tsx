...
export default class ItemRender extends React.Component<Props> {
  render() {
    ...
    return (
      <div className={classNames(`item-render ${ (item.dmg) ? item.dmg : '' }`, { masterwork: item.masterwork, exotic: item.isExotic })}>
        <div className='image'>
              ...
              return <div key={socketInfo.socketIndex} className={`plug-${index + 1}`}>
                  {
                    socketInfo.plug && category.category.categoryStyle !== DestinySocketCategoryStyle.Reusable &&
                      <div className="item-mod" style={{ backgroundImage: `url('https://www.bungie.net${socketInfo.plug.plugItem.displayProperties.icon}')`}} />
                  }
                </div>
            })
          }
