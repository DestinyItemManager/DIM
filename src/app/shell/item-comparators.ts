...
import { D2ItemRarityTiers } from 'app/search/d2-known-values';
...
const GROUP_BY_GETTERS_AND_COMPARATORS: {
  ...
  rarity: {
    ...
    getValue: (item) => D2ItemRarityTiers[item.rarity],
    getIcon: () => ({ type: 'none' }),
  },
...
const ITEM_COMPARATORS: {
  ...
  // exotic -> common
  rarity: reverseComparator(compareBy((item) => D2ItemRarityTiers[item.rarity])),
  // high -> low
