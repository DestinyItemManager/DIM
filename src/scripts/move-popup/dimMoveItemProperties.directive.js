...
function MoveItemProperties() {
  return {
    ...
    template: [
      ...
      '    </div>',
      '    <div ng-if="vm.featureFlags.reviewsEnabled && vm.item.totalReviews" class="item-review-average">',
      ...
      '  <div ng-if="vm.featureFlags.reviewsEnabled && vm.item.primStat.statHash === 368428387" class="move-popup-tabs">',
      ...
      '    <form ng-if="vm.featureFlags.tagsEnabled && vm.item.taggable" name="notes"><textarea name="data" translate-attr="{ placeholder: \'Notes.Help\' }" ng-maxlength="120" ng-model="vm.item.dimInfo.notes" ng-model-options="{ debounce: 250 }" ng-change="vm.updateNote()"></textarea></form>',
