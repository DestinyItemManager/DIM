...
function BungieService($rootScope, $q, $timeout, $http, $state, dimState, $translate) {
  ...
  function handleErrors(response) {
    ...
    if (response.status === 503 || response.status === 520 || response.status === 521 || response.status === 522 || response.status === 523 || response.status === 524 || response.status === 525 /* cloudflare */) {
