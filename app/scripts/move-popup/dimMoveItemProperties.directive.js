  ...
  function MoveItemProperties($sce) {
    return {
      ...
      template: [
        ...
        '         <span ng-if="stat.value === stat.equippedStatsValue && stat.equippedStatsName != \'Magazine\'" ng-show="{{ stat.bar }}" class="stat-box-inner" style="width: {{ stat.value }}%"></span>',
        '         <span ng-if="stat.value < stat.equippedStatsValue && stat.equippedStatsName != \'Magazine\'" ng-show="{{ stat.bar }}" class="stat-box-inner" style="width: {{ stat.value }}%"></span>',
        '         <span ng-if="stat.value < stat.equippedStatsValue && stat.equippedStatsName != \'Magazine\'" ng-show="{{ stat.equippedStatsValue - stat.value}}" class="stat-box-inner lower-stats" style="width: {{ stat.equippedStatsValue - stat.value }}%"></span>',
        '         <span ng-if="stat.value > stat.equippedStatsValue && stat.equippedStatsName != \'Magazine\'" ng-show="{{ stat.equippedStatsValue }}" class="stat-box-inner" style="width: {{ stat.equippedStatsValue }}%"></span>',
        '         <span ng-if="stat.value > stat.equippedStatsValue && stat.equippedStatsName != \'Magazine\'" ng-show="{{ stat.value - stat.equippedStatsValue}}" class="stat-box-inner higher-stats" style="width: {{ stat.value - stat.equippedStatsValue }}%"></span>',
        '         <span ng-if="stat.value < stat.equippedStatsValue && stat.equippedStatsName === \'Magazine\'" ng-hide="{{ stat.bar }}" class="lower-stats">{{ stat.value }}</span>',
        '         <span ng-if="stat.value > stat.equippedStatsValue && stat.equippedStatsName === \'Magazine\'" ng-hide="{{ stat.bar }}" class="higher-stats">{{ stat.value }}</span>',
        '         <span ng-if="stat.value === stat.equippedStatsValue" ng-hide="{{ stat.bar }}">{{ stat.value }}</span>',
        '       </span>',
        '         <span ng-if="stat.value < stat.equippedStatsValue" ng-show="{{ stat.bar }}" class="lower-stats stat-box-val">{{ stat.value }}</span>',
        '         <span ng-if="stat.value > stat.equippedStatsValue" ng-show="{{ stat.bar }}" class="higher-stats stat-box-val">{{ stat.value }}</span>',
        '         <span ng-if="stat.value === stat.equippedStatsValue" ng-show="{{ stat.bar }}" class="stat-box-val">{{ stat.value }}</span>',
        '    </div>',
        '    <div ng-if="vm.item.sort != \'Weapons\' || vm.item.owner === \'vault\'" class="stat-box-row">',
        '       <span class="stat-box-text"> {{ stat.name }} </span>',
        '       <span class="stat-box-outer">',
