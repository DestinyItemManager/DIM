  ...
  function MoveItemProperties() {
    return {
      ...
      template: [
        ...
        '  <div>',
        '    <a target="_new" href="http://db.destinytracker.com/inventory/item/{{vm.item.hash}}">{{vm.title}}</a>',
        '    <img class="title-element" ng-if=":: vm.item.dmg && vm.item.dmg !== \'kinetic\'" ng-src="/images/{{::vm.item.dmg}}.png"/>',
        '  </div>',
        '  <span ng-if="vm.light" ng-bind="vm.light"></span>',
        '  <span ng-if="vm.light">',
        '    <span ng-if="::vm.item.sort == \'Weapons\' || vm.item.type === \'Bounties\'"> | </span>',
        '  </span>',
        ...
        '  <span class="pull-right move-popup-info-detail">',
        '    <i ng-if="vm.item.lockable" class="fa" ng-class="{ \'fa-lock\': vm.item.locked, \'fa-unlock\': !vm.item.locked, locking: vm.locking }" ng-click="vm.setLockState(vm.item);"></i>',
        '    <span ng-click="vm.itemDetails = !vm.itemDetails;" ng-if="!vm.showDetailsByDefault && (vm.showDescription || vm.hasDetails) && !vm.item.classified">',
        '      <i class="fa" ng-class="{ \'fa-times-circle\': vm.itemDetails, \'fa-info-circle\': !vm.itemDetails }"></i>',
        '    </span>',
        '  </span>',
        '</div>',
