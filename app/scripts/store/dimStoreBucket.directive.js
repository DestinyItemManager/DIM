  ...
  function StoreBucket(dimStoreService, $window) {
    return {
      ...
      template: [
        ...
        '       drop-channel="{{::vm.dropChannel}}">',
        '    <dim-store-item ng-repeat="item in vm.items | equipped:true track by item.index" store-data="vm.store" item-data="item"></dim-store-item>',
        '  </div>',
        '  <div class="unequipped equippable" ui-on-drop="vm.onDrop($data, $event, false)" ',
