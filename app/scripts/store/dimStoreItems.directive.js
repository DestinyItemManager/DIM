  ...
  function StoreItems(dimStoreService, $window) {
    return {
      ...
      template: [
        ...
        '    <div ng-repeat="key in vm.keys track by key" ng-init="value = vm.categories[key]" class="section {{ key.toLowerCase() }}">',
        '      <div class="title">',
        '        <span>{{ key }}</span>',
        '        <span class="bucket-count" ng-if="vm.store.id === \'vault\'">{{ vm.sortSize[key] ? vm.sortSize[key] : 0 }}/{{ key === \'Weapons\' ? 36 : 24 }}  </span>',
        '      </div>',
        '      <div ng-repeat="type in value track by type" class="sub-section sort-{{ type.toLowerCase() }}" ng-class="vm.data[vm.orderedTypes[type]] ? \'\' : \'empty\'" ui-on-drop="vm.onDrop($data, $event, false)" drop-channel="{{ type }}">',
