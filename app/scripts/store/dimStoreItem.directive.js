...
(function() {
    ...
    function StoreItem(dimStoreService, ngDialog, dimLoadoutService) {
        return {
            ...
            template: [
                '<div ui-draggable="{{ (vm.item.type !== \'Lost Items\') && (vm.item.type !== \'Messages\')  }}" id="item-{{:: $id }}" drag-channel="{{ vm.item.type }}" title="{{ vm.item.primStat.value }} {{ vm.item.name }}" alt="{{ vm.item.primStat.value }} {{ vm.item.name }}" drag="\'item-\' + $id" class="item-container" ng-class="{ \'search-hidden\': !vm.item.visible, \'complete\': vm.item.complete }">',
                '  <div class="item col-xs-12">',
                '   <div ui-draggable="false" class="img itemimg" ng-class="{ \'how\': vm.item.inHoW }" ng-click="vm.clicked(vm.item, $event)"></div>',
        ...
        function Link(scope, element, attrs) {
            ...
            $('<img/>').attr('src', vm.item.icon).load(function() {
                $(this).remove(); // prevent memory leaks as @benweet suggested
                element[0].querySelector('.img')
                    .style.backgroundImage = 'url('+ vm.item.icon + ')';
            }).error(function() {
                $(this).remove(); // prevent memory leaks as @benweet suggested
                element[0].querySelector('.img')
                ...
                if (!_.isNull(dialogResult)) {
                    ...
                    if (!dimLoadoutService.dialogOpen) {
                        dialogResult = ngDialog.open({
                            ...
                            overlay: false,
                            className: 'panel panel-default ' + ((($('body').width() - $(element).offset().left - 450) < 0) ? ' panel move-popup-right' : ''),
                            showClose: false,
                            scope: scope
                        });
